...
const PO_STATUS_OPTIONS = ['', 'draft', 'pending', 'approved', 'received', 'cancelled'];
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Manage all orders sent to suppliers"
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                  {['PO Number', 'Supplier', 'Order Date', 'Expected Delivery', 'Total Amount', 'Status', 'Actions'].map(h => (
              ...
              <tbody>
                {orders.map((order) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 16px', fontSize: '13px', color: '#374151' }}>{order.supplier_name}</td>
                    <td style={{ padding: '14px 16px', fontSize: '13px', color: '#6b7280' }}>{formatDate(order.order_date)}</td>
                    <td style={{ padding: '14px 16px', fontSize: '13px', color: '#6b7280' }}>{formatDate(order.expected_delivery_date)}</td>
                    <td style={{ padding: '14px 16px', fontSize: '13px', fontWeight: '500', color: '#111827' }}>{formatCurrency(order.total_amount)}</td>
                    <td style={{ padding: '14px 16px' }}><StatusBadge status={order.status} type="po" /></td>
                    <td style={{ padding: '14px 16px' }}>
              ...
              <div style={{ padding: '14px 16px', borderTop: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                    style={{ padding: '6px 14px', fontSize: '13px', border: '1px solid #e0e0e0', borderRadius: '6px', background: page === 1 ? '#f9fafb' : '#fff', cursor: page === 1 ? 'not-allowed' : 'pointer', color: '#374151' }}>
                    ← Prev
                  </button>
                  ...
                  <button disabled={page === pagination.total_pages} onClick={() => setPage(p => p + 1)}
                    style={{ padding: '6px 14px', fontSize: '13px', border: '1px solid #e0e0e0', borderRadius: '6px', background: page === pagination.total_pages ? '#f9fafb' : '#fff', cursor: page === pagination.total_pages ? 'not-allowed' : 'pointer', color: '#374151' }}>
                    Next →
                  </button>
                </div>
              </div>
      ...
      <ConfirmDialog
        ...
        message="This action cannot be undone. The purchase order and all its items will be permanently deleted."
