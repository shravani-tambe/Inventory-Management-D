...
import { SO_STATUSES } from '../../utils/constants';
...
const SalesOrderDetailPage = () => {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      const res = await salesOrderApi.updateStatus(id, selectedStatus);
  ...
  return (
    <div>
      <PageHeader title={order.so_number} subtitle={`Created ${formatDateTime(order.created_at)}`} showBack />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '20px' }}>
        <div>
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '24px', marginBottom: '20px' }}>
            ...
            <InfoRow label="SO Number" value={order.so_number} />
            <InfoRow label="Customer Name" value={order.customer_name} />
            <InfoRow label="Customer Email" value={order.customer_email} />
            <InfoRow label="Customer Phone" value={order.customer_phone} />
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', padding: '20px' }}>
            ...
            >
              {Object.keys(SO_STATUSES).map(s => (
                <option key={s} value={s}>{SO_STATUSES[s].label}</option>
              ))}
            </select>
            <button
              onClick={handleStatusUpdate}
              disabled={updatingStatus || selectedStatus === order.status}
              style={{ width: '100%', padding: '9px', background: updatingStatus || selectedStatus === order.status ? '#93c5fd' : '#2563eb', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', cursor: updatingStatus || selectedStatus === order.status ? 'not-allowed' : 'pointer' }}
            >
